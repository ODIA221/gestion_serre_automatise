...
const TableauDB = () => {
  const [Arrosage, setarrosage] = useState(1)

  const socket = io('ws://localhost:5000');
  socket.emit('Arrosage', (data) => {
    setarrosage(data);
    console.log(data);
   
    socket.emit('etatToit', (data) => {
      console.log(data);
}) 
    ...
    <div id='containerGeneralDash2'>
      ...
      <div class='containerDash2C '>
          ...
          <div class='elementDash2 '>
            ...
            <button class="btn btn-success" onClick={() => Arrosage4(arrosage)} > Ouvrir</button>
            </div>
          </div>
      </div>
    </div>
