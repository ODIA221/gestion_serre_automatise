...
function Historiques() {
  ...
  const handleSearch = () => {
    axios.get(`http://localhost:5000/api/recu?jour=${searchDate}`)
      .then(response => {
        setHistoriqueData(response.data);
      })
      .catch(error => {
  ...
  return (
    <div className="container">
      ...
      <div className="search">
        <input type="text" placeholder="Rechercher par date (AAAA-MM-JJ)" value={searchDate} onChange={(e) => setSearchDate(e.target.value)} />
